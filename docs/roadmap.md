...
| G4 | Entrega e storage do modelo: embutido vs download+cache, `mmap` no load, Play Asset Delivery p/ modelos grandes | médio | 🟡 quase — **embutido** ✅ (asset no bundle, device-provado em G1-G3); **download+cache+verify** ✅ `native/model_store.py` `ensure_model` (cache-first, sha256, off-loop via `to_thread`, stdlib urllib/hashlib → roda no device; `mmap` é implícito no load-by-path da AAR). 6 testes (file://). Falta: device-verify do download (adb-reverse localhost → cache → classifica) + wiring no `examples/visionspike` — delegado. Play Asset Delivery = futuro p/ modelos grandes |
