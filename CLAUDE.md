...
| G4 | Entrega e storage do modelo: embutido vs download+cache, `mmap` no load, Play Asset Delivery p/ modelos grandes | 🟡 quase | **Embutido** ✅ (asset no bundle, device-provado G1-G3). **Download+cache+verify** ✅ `native/model_store.py` `ensure_model(url, dest_dir, *, sha256, filename)`: cache-first (retorna path cacheado hash-verificado), senão baixa 1x (streamed, off-loop via `asyncio.to_thread`), verifica sha256, rename atômico. Stdlib urllib/hashlib → roda no interpretador embarcado sem módulo nativo extra; `file://` suportado. `mmap` no load é implícito (AAR carrega por path → ORT mmapa). `ModelStoreError` (download_failed/hash_mismatch). 6 testes (file://). **Pendente:** device-verify do download (adb-reverse localhost → cache → classifica) + wiring no exemplo (delegado); Play Asset Delivery = futuro |
