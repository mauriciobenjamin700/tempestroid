...
| G2 | Caminho de imagem sem OpenCV (Pillow / `BitmapFactory` do host) + pré/pós em `numpy` | ⏳ planejado | imagem câmera/galeria → tensor → inferência sem `opencv-python` na APK |
| G3 | (opcional) `pandas` no device — feature-engineering tabular | ⏳ planejado | `import pandas` + pipeline tabular roda no aparelho |
| G4 | (opcional) `scipy`+`scikit-learn` no device — ML clássico (calcanhar: Fortran/LAPACK+OpenMP) | ⏳ planejado | `import sklearn`; modelo sklearn faz `predict` no aparelho |
