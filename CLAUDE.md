...
| G6 | (opcional) `scipy`+`scikit-learn`+`scikit-image` no device — ML clássico + processamento de imagem (calcanhar: Fortran/LAPACK+OpenMP; skimage gated atrás do scipy) | ✅ done (emulador) | **scipy 1.18.0 + scikit-learn 1.9.0 importam, `fit`/`predict` + `scipy.linalg.solve` rodam no emulador x86_64** (`examples/sklearnspike`: `LogisticRegression.predict([2,8])=[0,1]`, `solve=[2.0,3.0]`, screenshot). Calcanhar sumiu upstream: cross-compilam clang-only ZERO Fortran (OpenBLAS `NOFORTRAN=1 C_LAPACK=1`, scipy `-Dwithout-fortran`), OpenMP via NDK `libomp`. Staging opt-in `make stage-science` (2 wheels + deps puras joblib/threadpoolctl/narwhals). **3 fixes de packaging no `build.gradle.kts`:** trims G7(`numpy/f2py`)+F6(`pydoc`/`_pyrepl`/`doctest`) quebravam scipy/sklearn (importam em runtime) → revertidos; `.gz` rename p/ datasets `.csv.gz` do sklearn passarem no asset-merger; excludes de tests scipy/sklearn/joblib. APK ~156M (G7 encolhe). Pendente: skimage + rebuild arm64 |
