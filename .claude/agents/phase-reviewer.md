...
2. **Both renderers green.** A feature that landed Qt but not Compose (or vice-versa), or a `Style` field added to one translator and not the other without a documented divergence, is a defect — the core rule is one reconciler, two mirrored renderers.
   - **Device evidence is a hard gate (when a device is connected).** If the device-verifier returned `deviceConfirmed: false`, or reported ANY bug / un-exercised "feito quando" device item, the verdict is **BLOQUEADO** — never APROVADO. Green unit tests + a clean gate do NOT substitute for the on-device proof the dual-verification rule requires; the device leaf (Compose + JNI + bridge) is exactly what pytest cannot reach. Treat "device dropped offline mid-run" as an un-met gate, not a pass.
