...
def test_remove_purges_tracked_click_connection():
    """Removing a button drops its tracked click connection (no id leak)."""
    renderer = QtRenderer()
    old = build(
        Column(children=[Text(content="t"), Button(label="x", on_click=lambda: None)])
    )
    renderer.mount(old)
    button = renderer.root_widget.findChild(QPushButton)
    assert button is not None
    conns = renderer._click_conns  # pyright: ignore[reportPrivateUsage]
    assert id(button) in conns

    # Diff to a tree without the button → Remove patch → connection purged.
    new = build(Column(children=[Text(content="t")]))
    renderer.apply(diff(old, new))
    assert id(button) not in conns


def test_replace_purges_nested_click_connection():
    """Replacing a subtree purges connections for buttons nested inside it."""
    renderer = QtRenderer()
    old = build(
        Column(children=[Row(children=[Button(label="x", on_click=lambda: None)])])
    )
    renderer.mount(old)
    button = renderer.root_widget.findChild(QPushButton)
    assert button is not None
    ...
    # Replace the Row with a Text → the whole subtree (incl. the button) is
    # discarded; the nested connection must be purged too.
    new = build(Column(children=[Text(content="z")]))
    renderer.apply(diff(old, new))
    assert id(button) not in conns
